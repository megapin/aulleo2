...
const onOrder = async () => {
	let {id, market, price: avg_buy_price} = au.market
	let {side, volume, price, ord_type} = au.market
	const body = ord_type == 'market' 
		? {market, side, volume, ord_type}
	...
	await pb.update('upbit_orders', id, {uuid: res.uuid, status: 'completed', manual: true})
...
const onVol = (e) => {
	...
	au.market.pct = ~~(e.target.value/au.market.balance * 100)
}
const onPct = (e) => {
	au.market.total = au.market.balance * e.target.value/100 * au.market.price
	au.market.volume = au.market.balance * e.target.value/100
}
const onTot = (e) => {
	au.market.volume = e.target.value/au.market.avg_buy_price
	const marketTot = au.market.balance*au.market.avg_buy_price
	au.market.pct = (e.target.value)/marketTot*100
}
const onRadio = (e) => {
	...
	<div class="mx-auto">
		<div class="flex justify-center">
			<div class="flex flex-wrap gap-2">
				...
				<label>지정가</label>
				<input v-model="au.market.ord_type" type="radio" value="market" @input="au.market.price = 0" />
				<label>시장가</label>
		...
		<div class="flex items-center gap-2 mt-2">
			...
			<input v-model="au.market.price" @input="" type="number" :step="getUnit(au.market?.trade_price)" 
				:disabled="au.market.ord_type == 'market'" :maxFractionDigits="5" class="w-[15rem]" /> 
		</div>
		<div class="flex items-center gap-2 mt-2">
			<label class="w-[6rem]">주문수량</label>
			<input v-model="au.market.volume" @input="onVol" type="number" :min="0" :max="myKrw.balance/au.market.price" 
				:step="getVolStep(au.market.trade_price)" class="w-[15rem]" />
			<input v-model="au.market.pct" @input="onPct" type="number" :min="0" :max="100" :step="1" class="w-[7rem]" />%
		</div>
		<div class="flex items-center gap-2 mt-2">
			<label class="w-[6rem]">주문총액</label>
			<input v-model="au.market.total" @input="onTot" type="number" class="w-[15rem]" :min="10000" :max="au.market.balance*au.market.avg_buy_price"
				:step="10000" :disabled="['market','price'].includes(au.market.ord_type)" />
		</div>
		<div class="flex items-center gap-2 mt-4">
			<span @click="onCalc" class="w-[6rem] cupo">🎛</span>
			<button @click="onOrder" :disabled="!au.market.market" class="w-[24rem] p-1 px-2 rounded text-white bg-blue-500">매도</button>
		</div>
	</div>
